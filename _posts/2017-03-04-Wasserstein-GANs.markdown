...
to find a way to measure how far away $$P_{\theta}$$ is from $$P_{r}$$. WGANs use the Wasserstein-1 to measure
...
[$$K$$-Lipschitz](https://en.wikipedia.org/wiki/Lipschitz_continuity), fortunately, we can make it so just by restricting
...
$$c$$: clipping parameter 
 ...
 6. > > > Update $$w$$:$$\quad w \leftarrow w + \alpha \times \text{RMSProp}(w, \mathcal{G}_{w})$$
 7. > > > Clip $$w$$:$$\quad w \leftarrow \text{clip}(w, -c, c) $$
 8. > > **end for**
 9. > > Sample {$$z^{(i)}$$} $$ \sim p(z)$$ a batch of priors
 10. > > Calculate gradient w.r.t $$\theta$$:  $$\quad \mathcal{G}_{\theta} \leftarrow \nabla_{\theta} \left[\frac{1}{m}\sum\limits_{i = 1}^{m}f_{w}(x_{real}^{(i)}) - \frac{1}{m}\sum\limits_{i = 1}^{m}f_{w}(g_{\theta}(z^{(i)}))\right]$$
 11. > > Update $$\theta$$:$$\quad \theta \leftarrow \theta - \alpha \times \text{RMSProp}(\theta, \mathcal{G}_{\theta})$$
 12. > **end while**
