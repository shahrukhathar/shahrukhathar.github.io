...
For any given dgerading transformation $$\mathcal{T}$$ - which can be multiplication with a binary mask for inpainting, the downsampling kernel for super-resolution or  graying for colorization - we carry out the following optimization to restore images
...
Figures 3 and 4 show qualitative and quantitative results of inpainting, super-resolution and colorization using a LCM on the CelebA dataset. Perceptual metrics and results on more datasets can be found in the [paper](toNoWhere).
